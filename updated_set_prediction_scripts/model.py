...
class SlotAttention(layers.Layer):
  ...
  def __init__(self, num_iterations, num_slots, slot_size, mlp_hidden_size,
    ...
    self.dense_proj = tf.keras.layers.Dense(units=self.slot_size)
    ...
    #self.state_size = tf.TensorShape([self.slot_size*3])
    #self.output_size=self.slot_size/3
  ...
  def call(self, inputs):
    ...
    for _ in range(self.num_iterations):
      ...
      #updates = self.dense_proj(updates)
...
class SlotAttentionClassifier(layers.Layer):
  ...
  def __init__(self, resolution, num_slots, num_iterations):
    ...
    self.encoder_pos = SoftPositionEmbed(64, (40, 60))
