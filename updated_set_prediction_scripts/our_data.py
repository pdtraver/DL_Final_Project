...
def preprocess_clevr(features, resolution, apply_crop=False,
  ...
  if get_properties:
    ...
    #UPDATE: our data has coords as float64, need float32
    ...
    features = {"image": image, "file_name": features["file_name"]}

  return features


def build_clevr(split, resolution=(160, 240), shuffle=False, max_n_objects=10,
                num_eval_examples=512, get_properties=True, apply_crop=False, unlabeled=False):
    """Build CLEVR dataset."""
    if split == "train" or split == "train_eval":
        #UPDATE: load wherever our newly formatted dataset is stored
        # ds = tfds.load("label_dataset:1.0.0",  data_dir='/scratch/kmk9461/google-research/slot_attention', split="train", shuffle_files=shuffle)
        ds = tfds.load("label_dataset:1.0.0",  data_dir='/scratch/pdt9929/DL_Final_Project/google_research/slot_attention', split="train", shuffle_files=shuffle)
        # if split == "train":
        ...
        #UPDATE: again point to new dataset
        # ds = tfds.load("label_dataset:1.0.0",  data_dir='/scratch/kmk9401/google-research/slot_attention', split=split, shuffle_files=shuffle)
        ds = tfds.load("label_dataset:2.0.0",  data_dir='/scratch/pdt9929/DL_Final_Project/google_research/slot_attention', split=split, shuffle_files=shuffle)
    elif split == "test":
        #UPDATE: again point to new dataset
        # ds = tfds.load("label_dataset:1.0.0",  data_dir='/scratch/kmk9401/google-research/slot_attention', split=split, shuffle_files=shuffle)
        ds = tfds.load("label_dataset:5.0.0",  data_dir='/scratch/pdt9929/DL_Final_Project/google_research/slot_attention', split=split, shuffle_files=shuffle)
    elif split == "prediction":
        #UPDATE: again point to new dataset
        # ds = tfds.load("label_dataset:1.0.0",  data_dir='/scratch/kmk9401/google-research/slot_attention', split=split, shuffle_files=shuffle)
        ds = tfds.load("label_dataset:11.0.0",  data_dir='/scratch/pdt9929/DL_Final_Project/google_research/slot_attention', split="test", shuffle_files=shuffle)
