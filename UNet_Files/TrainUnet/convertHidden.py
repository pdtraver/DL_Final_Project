...
checkpoint_path = "/scratch/pdt9929/TrainUnet/Unet_checkpoint_epoch_89.pt"  # Replace xx with the actual epoch number
...
dataset = CLEVRDataset(path='/scratch/pdt9929/DL_Final_Project/dataset/unlabeled')
...
count_videos = 0
slices = []
for images, folder in tqdm(dataloader):
    if not all(elem == folder[0] for elem in folder):
        raise ValueError("All elements in the folder array must be the same value.")
        
    count_videos += 1
    save_path = os.path.join(folder[0], 'mask.npy')
    if os.path.isdir(save_path):
        print('it thinks it is a dir')
        os.rmdir(save_path)
    
    if not os.path.isfile(save_path):
        print('it thinks it is a file')
        ...
        with open(os.path.join(save_path), 'wb') as f:
            np.save(f, argmax_result.cpu().detach().numpy())  # Save the outputs as a NumPy array
    
    else:
        argmax_result = torch.tensor(np.load(save_path))

    slice_22 = argmax_result
    slices.append(slice_22.cpu().detach().numpy())
...
# %%
save_path = "/scratch/pdt9929/TrainUnet/unlabeled_predictions_unet.npy"
